#AN2 models - FILE 2: Format results, build plots, export to Excel
...
load(file.path(data_out, "AN2_model_outputs.RData"))

# Load saved models (needed for prediction plots)
all_models <- readRDS(file.path(data_out, "all_models_auto.RData"))
...
write_list_to_xlsx(
  ...
  path = file.path(output, "all_results_combined3.xlsx")
)
