...
# Use a helper function so that the current month can be changed to 6 months ago and rerun - for SA
get_comparison_months <- function(data, offset_months = 0) {
  
  latest_month <- floor_date(max(data$Date, na.rm = TRUE), "month") %m-% months(offset_months)
...
# Run analysis inside function, so it can be run for main vs SA

run_analysis <- function(data, comparison_labels, output_suffix = "") {
...
write_list_to_xlsx(
  ...
  path = file.path(output, "all_results_combined3_sa.xlsx")
)
