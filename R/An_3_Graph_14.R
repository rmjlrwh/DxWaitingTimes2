...
# Ensure every sub-ICB has a row for every TestName, so boundaries
# are always drawn even when data is missing
all_tests     <- unique(all_compare2_df$TestName)
all_nhscodes  <- unique(sub_icb_boundaries$NHSCode_PostMerge)
complete_grid <- expand.grid(
  NHSCode_PostMerge = all_nhscodes,
  TestName          = all_tests,
  stringsAsFactors  = FALSE
)

all_compare2_df_complete <- complete_grid |>
  left_join(all_compare2_df, by = c("NHSCode_PostMerge", "TestName"))

map_data <- sub_icb_boundaries |>
  left_join(all_compare2_df_complete, by = "NHSCode_PostMerge")
...
rel_change_levels <- c(rel_change_labels, "No significant difference", "No data")
...
prob_labels  <- c("< 10%", "10% to 19%", "20% to 29%", "30% to 39%", "40% to 49%", "50% +", "No data")
prob_colours <- c(
  "< 10%"      = "#73B3D8",
  "10% to 19%" = "#BDD7E7",
  "20% to 29%" = "#E7EFF6",
...
categorise_rel_change <- function(rel_change, change_desc) {
  ...
  cat[!is.na(change_desc) & change_desc == "No significant difference"] <- "No significant difference"
  cat[is.na(cat)] <- "No data"
...
map_data <- map_data |>
  mutate(
    ...
    Predicted_prob_prev_cat   = cut(.data[[prob_prev_col]],   breaks = prob_breaks, labels = prob_labels[prob_labels != "No data"], right = FALSE),
    Predicted_prob_latest_cat = cut(.data[[prob_latest_col]], breaks = prob_breaks, labels = prob_labels[prob_labels != "No data"], right = FALSE)
  ) |>
  mutate(
    Predicted_prob_prev_cat   = forcats::fct_na_value_to_level(Predicted_prob_prev_cat,   "No data"),
    Predicted_prob_latest_cat = forcats::fct_na_value_to_level(Predicted_prob_latest_cat, "No data")
  )
...
change_summary_table <- change_summary_table |>
  order_by_test(test_col = "Test")
...
wb <- loadWorkbook(file.path(output, paste0(run_label,"_all_results_combined3.xlsx")))
...
saveWorkbook(wb, file.path(output, paste0(run_label,"_all_results_combined3.xlsx")),
             overwrite = TRUE)